...
export default function Index({ rakList, lokasiList = [] }: Props) {
                    ...
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-zinc-700">
                        <thead className="bg-gray-100 dark:bg-zinc-800">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-left text-xs font-medium uppercase">Nama Rak</th>
                                <th className="px-6 py-3 text-left text-xs font-medium uppercase">Baris</th>
                                <th className="px-6 py-3 text-left text-xs font-medium uppercase">Kode Rak</th>
                        ...
                        <tbody className="divide-y divide-gray-200 dark:divide-zinc-700">
                            {rakList.map((rak, index) => (
                                <tr key={rak.id} className="hover:bg-gray-50 dark:hover:bg-zinc-800">
                                    ...
                                    <td className="px-6 py-4 text-sm">{rak.lokasi?.nama || '-'}</td>
                                    <td className="px-6 py-4 text-sm">{rak.nama_rak}</td>
                                    <td className="px-6 py-4 text-sm">{rak.baris || '-'}</td>
                                    <td className="px-6 py-4 text-sm">{rak.kode_rak}</td>
                                    <td className="flex gap-3 px-6 py-4 text-sm whitespace-nowrap">
                                        <button onClick={() => handleEdit(rak)} className="text-blue-600 hover:text-blue-800 dark:text-blue-400">
                                            <Edit3 size={18} />
                                        </button>
                                        <button onClick={() => handleDelete(rak.id)} className="text-red-600 hover:text-red-800 dark:text-red-400">
                                            <Trash2 size={18} />
                                        </button>
                                    </td>
                                </tr>
