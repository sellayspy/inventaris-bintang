...
export function DetailStokModal({ isOpen, onClose, item, details, isLoading }: DetailStokModalProps) {
    ...
    const getStatusBadge = (status: string) => {
        ...
        return `px-2 py-0.5 text-xs font-medium rounded-full ${styles[status.toLowerCase()] || 'bg-gray-100 text-gray-800'}`;
    };
                    ...
                    {isLoading ? (
                        ...
                        <table className="w-full text-sm">
                            <thead className="bg-gray-50">
                                <tr>
                                    <th className="p-2 text-left font-medium text-gray-600">No</th>
                                    <th className="p-2 text-left font-medium text-gray-600">Serial Number</th>
                                ...
                                {details?.length > 0 ? (
                                    details.map((barang, index) => (
                                        <tr key={barang.id}>
                                            ...
                                            <td className="p-2 font-mono">{barang.serial_number}</td>
                                            <td className="p-2">
